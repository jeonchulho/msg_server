...
## Ramp-up Matrix (권장 단계별 파라미터)

| Stage | profile | k6_vus | k6_duration | 목표 |
|---|---|---:|---|---|
| A (스모크 성능) | msa | 300 | 5m | 워크플로우/아티팩트/기본 임계치 정상 동작 확인 |
| B (기준선) | msa | 500 | 10m | p95/p99 변동성 확인, 병목 1차 식별 |
| C (중간 부하) | msa | 1000 | 15m | 안정 구간 상한 확인, 실패율 추세 점검 |
| D (고부하 진입) | msa | 2000 | 20m | 스케일 한계/타임아웃/큐 적체 확인 |

상승 규칙(권장):
- 직전 단계에서 `failed_rate < 2%` 이고 p95/p99 급등이 없을 때만 다음 단계로 진행
- 실패 시 동일 단계 1회 재실행 후 원인 분석 (우선순위: DBMan → PostgreSQL → Redis/MQ)
- 동일 단계 2회 연속 실패 시 상위 단계 진행을 중단하고 개선 항목 반영 후 재시작
